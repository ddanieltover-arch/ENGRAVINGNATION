...
export const metadata: Metadata = buildPageMetadata({
  title: 'Automotive Engraving Glossary',
  description:
    'Definitions for precision engraving, OEM fitment, mirror caps, and custom emblem terminology used in truck customization.',
  path: '/glossary',
});
      ...
      <div className="container mx-auto px-4">
        <div className="max-w-4xl mx-auto">
          <header className="mb-16">
            ...
            <AnswerCapsule className="mb-8 p-8 rounded-3xl bg-brand-gold/[0.03] border border-brand-gold/20">
              This glossary defines core automotive engraving terms — including OEM fitment, precision engraving,
              mirror caps, and grille emblems — so you can choose the right custom upgrade with confidence.
            </AnswerCapsule>
          ...
          <dl className="space-y-12">
            {glossaryTerms.map((item) => (
              <div key={item.term} className="group p-8 rounded-3xl bg-white/[0.03] border border-white/5 hover:border-brand-gold/20 transition-all duration-500">
                <dt className="text-2xl font-heading font-black uppercase italic tracking-tight text-white mb-4 group-hover:text-brand-gold transition-colors">
                  {item.term}
                </dt>
                <dd className="text-white/60 text-lg font-light leading-relaxed">
                  {item.definition}
                </dd>
              </div>
            ))}
          ...
          <footer className="mt-24 p-12 rounded-3xl bg-brand-gold/5 border border-brand-gold/10 text-center">
            ...
            <Link href="/products" className="inline-block px-10 py-4 rounded-full bg-brand-gold text-black font-black uppercase tracking-widest italic hover:shadow-[0_0_30px_rgba(212,160,23,0.3)] transition-all">
              Shop the Collection
            </Link>
          </footer>
        </div>
